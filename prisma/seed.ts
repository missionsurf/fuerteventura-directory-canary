...
async function main() {
  ...
  const bizUser2Password = await bcrypt.hash("demo123", 10)
  const bizUser2 = await prisma.user.upsert({
    where: { email: "surf@example.com" },
    update: {},
    create: { email: "surf@example.com", name: "Surf School Owner", password: bizUser2Password, role: "business" },
  })
  await prisma.business.upsert({
    ...
    create: {
      ...
      description: "Professional surf lessons for all levels. Certified instructors, quality equipment, and lessons in English and Spanish. Group and private sessions available.",
  ...
  const bizUser3Password = await bcrypt.hash("demo123", 10)
  const bizUser3 = await prisma.user.upsert({
    where: { email: "hotel@example.com" },
    update: {},
    create: { email: "hotel@example.com", name: "Hotel Owner", password: bizUser3Password, role: "business" },
  })
  await prisma.business.upsert({
  ...
  const bizUser4Password = await bcrypt.hash("demo123", 10)
  const bizUser4 = await prisma.user.upsert({
    where: { email: "kite@example.com" },
    update: {},
    create: { email: "kite@example.com", name: "Kite Center Owner", password: bizUser4Password, role: "business" },
  })
  await prisma.business.upsert({
    ...
    create: {
      ...
      description: "World-class kitesurfing center on Flag Beach. IKO certified instructors. Equipment rental and lessons for all levels. Best winds on the island guaranteed.",
